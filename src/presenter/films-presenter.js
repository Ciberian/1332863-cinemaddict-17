...
export default class FilmsPresenter {
  ...
  init = (filmsContainer) => {
    ...
    for (let i = 0; i < FILMS_AMOUNT_DISPLAYED; i++) {
      render(new FilmCardView(), this.filmsContainerMainListComponent.getElement());
    }
    ...
    for (let i = 0; i < RATED_FILMS_DISPLAYED; i++) {
      render(new FilmCardView(), this.filmsContainerTopListComponent.getElement());
    }
    ...
    for (let i = 0; i < COMMENTED_FILMS_DISPLAYED; i++) {
      render(new FilmCardView(), this.filmsContainerCommentedListComponent.getElement());
    }
  };
}
